function filename=getfilename(option,prefix)
...
filename=strcat(option.sys,'_',text{1},'_E',num2str(option.ldeg,'%4.1f'),'N',num2str(option.pdeg,'%3.1f'),...
    '_',option.tropo,'_sdp',num2str(stdp),'mm_sdc',num2str(stdc),'cm_sdi',num2str(stdi),'mm');
if exist('prefix','var')
   filename = strcat(prefix,'_',filename);
end